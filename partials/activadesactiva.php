...
if($activacion==0){
  $query1 = "UPDATE usuario set activo='no activo' where user_register_id_usuario = $id;";
   $insertado =mysqli_query($db, $query1);
  echo "Ya no estas activo";
}else{

  if($nombre <> NULL and $apellido_pat<>NULL and $apellido_mat<>NULL and $edad <> NULL and $telefono <> NULL and $celular <> NULL and $linkedin <> null
  and $sueldo_ac <> NULL and $sueldo_des <> NULL and $descripcionhab <> NULL and $descripcionper <> NULL and $horario <> NULL
  ...
  $query1 = "UPDATE usuario set activo='activo' where user_register_id_usuario = $id;";
   $insertado =mysqli_query($db, $query1);
  echo "Ya estas activo";
} else{
  echo "Debes llenar todos los campos para activarte";
}
